        ...
        socket.on('poll:create', (data, callback) => {
            try {
                ...
                if (!roomId || !question || !options || !socket.participantId) {
                    if (callback) callback({ success: false, error: 'Invalid poll data' });
                    return;
                }

                // Note: The maintainer moved activePolls into the registry.
                // You may need to add getActivePolls and setActivePolls to RoomRegistry.ts
                const activePollsMap = deps.registry.getActivePolls(roomId) || new Map<string, ActivePoll>();
                ...
                deps.registry.emitToFullAccessParticipants(roomId, 'poll:new', payloadFE);
            } catch (error) {
                loggers.socketHandler.warn('poll:create failed: %s', (error as Error).message);
        ...
        socket.on('poll:vote', (data, callback) => {
            try {
                ...
                if (!roomId || !pollId || !optionId || !socket.participantId) {
                    if (callback) callback({ success: false, error: 'Invalid vote data' });
                    return;
                }
                ...
                deps.registry.emitToFullAccessParticipants(roomId, 'poll:update', payloadFE);
            } catch (error) {
                loggers.socketHandler.warn('poll:vote failed: %s', (error as Error).message);
            }
        });
    };
}